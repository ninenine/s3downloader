...
func (u *UIManager) StartDownload() {
	...
	go u.downloadFiles(bucket, prefix, downloadPath)
}

func (u *UIManager) downloadFiles(bucket, prefix, downloadPath string) {
	...
	err := u.downloader.ListAndDownloadObjects(ctx, bucket, prefix, downloadPath, u.progressChan)

	close(u.progressChan)

	<-u.doneChan // Wait for the progress update goroutine to finish
...
func (u *UIManager) updateProgress(progress progress.Progress) {
	...
	u.components.StatusLabel.SetText(fmt.Sprintf("Files found: %d, Downloaded: %d, Skipped: %d", filesFound, filesDownloaded, progress.FilesSkipped))
